...
bool FubiOpenNISensor::initWithOptions(const Fubi::SensorOptions& options)
{
	...
	m_UserGenerator.GetSkeletonCap().SetSkeletonProfile((XnSkeletonProfile) options.m_profile);

	// Default is no smoothing (because you better do it yourself adapted to the task you have)
	m_UserGenerator.GetSkeletonCap().SetSmoothing(options.m_smoothing);
	...
	// Update the options
	updateOptions();
	// No getter for profile and smoothing in OpenNI
	m_options.m_profile = options.m_profile;
	m_options.m_smoothing = options.m_smoothing;
...
bool FubiOpenNISensor::initFromXml(const char* xmlPath, Fubi::SkeletonTrackingProfile::Profile profile /*= Fubi::SkeletonTrackingProfile::ALL*/,
	bool mirrorStreams /*= true*/, float smoothing /*= 0*/)
{
	...
	m_UserGenerator.GetSkeletonCap().SetSkeletonProfile((XnSkeletonProfile) profile);

	// Default is no smoothing (because you better do it yourself adapted to the task you have)
	m_UserGenerator.GetSkeletonCap().SetSmoothing(smoothing);
	...
	// Update the options
	updateOptions();
	// No getter for profile and smooting in OpenNI
	m_options.m_profile = profile;
	m_options.m_smoothing = smoothing;
